...
class VictimCompensationPage extends StatefulWidget {
  const VictimCompensationPage({super.key});

  @override
  State<VictimCompensationPage> createState() => _VictimCompensationPageState();
}

class _VictimCompensationPageState extends State<VictimCompensationPage> {
  List<Map<String, dynamic>> victimCompensations = [];
  bool isLoading = true;
  ...
  void initState() {
    super.initState();
    fetchVictimCompensations();
  }

  Future<void> fetchVictimCompensations() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // First, get all tags where victim_compensation is true
        final victimCompensationTags = tags.where((tag) => tag['victim_compensation'] == true).toList();
        
        // Get the uploads_ids from these victim_compensation tags
        final victimCompensationUploadIds = victimCompensationTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${victimCompensationTags.length} victim compensation tags'); // Debug print
        print('Victim Compensation upload IDs: $victimCompensationUploadIds'); // Debug print

        // Get all uploads that match these ids
        final filteredVictimCompensations = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return victimCompensationUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredVictimCompensations.length} victim compensations'); // Debug print
        
        // Sort victim compensations by upload date (newest first)
        filteredVictimCompensations.sort((a, b) {
        ...
        setState(() {
          victimCompensations = List<Map<String, dynamic>>.from(filteredVictimCompensations);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching victim compensations: $e');
      setState(() {
        isLoading = false;
      });
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (victimCompensations.isEmpty)
                  const Center(child: Text('No victim compensation records found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    padding: EdgeInsets.zero,
                    itemCount: victimCompensations.length,
                    itemBuilder: (context, index) {
                      final victimCompensation = victimCompensations[index];
                      return Container(
                          ...
                          child: InkWell(
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => DisplaycontentPage(
                                    achievement: victimCompensation,
                                    categoryName: 'Victim Compensation',
                                    currentIndex: index,
                                    itemsList: victimCompensations,
                                  ),
                                ),
                              );
                            },
                            child: Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  if (victimCompensation['first_picture'] != null)
                                    ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: CachedNetworkImage(
                                        imageUrl: '${baseUrl}/storage/${victimCompensation['first_picture']['picture_path']}',
                                  ...
                                  Text(
                                    'Uploaded on: ${victimCompensation['upload_date']}',
                                    style: TextStyle(
