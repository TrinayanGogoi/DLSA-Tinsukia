...
class JuvenileJusticePage extends StatefulWidget {
  const JuvenileJusticePage({super.key});

  @override
  State<JuvenileJusticePage> createState() => _JuvenileJusticePageState();
}

class _JuvenileJusticePageState extends State<JuvenileJusticePage> {
  List<Map<String, dynamic>> juvenileJusticeItems = [];
  bool isLoading = true;
  ...
  void initState() {
    super.initState();
    fetchJuvenileJusticeItems();
  }

  Future<void> fetchJuvenileJusticeItems() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // Get all tags where juvenile_justice is true
        final juvenileJusticeTags = tags.where((tag) => tag['juvenile_justice'] == true).toList();
        
        // Get the uploads_ids from these juvenile justice tags
        final juvenileJusticeUploadIds = juvenileJusticeTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${juvenileJusticeTags.length} juvenile justice tags');
        print('Juvenile justice upload IDs: $juvenileJusticeUploadIds');

        // Get all uploads that match these ids
        final filteredJuvenileJusticeItems = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return juvenileJusticeUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredJuvenileJusticeItems.length} juvenile justice items');
        
        // Sort juvenile justice items by upload date (newest first)
        filteredJuvenileJusticeItems.sort((a, b) {
        ...
        setState(() {
          juvenileJusticeItems = List<Map<String, dynamic>>.from(filteredJuvenileJusticeItems);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching juvenile justice items: $e');
      setState(() {
        isLoading = false;
      });
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (juvenileJusticeItems.isEmpty)
                  const Center(child: Text('No juvenile justice items found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    padding: EdgeInsets.zero,
                    itemCount: juvenileJusticeItems.length,
                    itemBuilder: (context, index) {
                      final item = juvenileJusticeItems[index];
                      return Container(
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => DisplaycontentPage(
                                    achievement: item,
                                    categoryName: 'Juvenile Justice',
                                    currentIndex: index,
                                    itemsList: juvenileJusticeItems,
                                  ),
                                ),
                              );
                            },
