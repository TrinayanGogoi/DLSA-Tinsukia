  ...
  Future<void> fetchNotices() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // First, get all tags where notice is true
        final noticeTags = tags.where((tag) => tag['notice'] == true).toList();
        
        // Get the uploads_ids from these notice tags
        final noticeUploadIds = noticeTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${noticeTags.length} notice tags'); // Debug print
        print('Notice upload IDs: $noticeUploadIds'); // Debug print

        // Get all uploads that match these ids
        final filteredNotices = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return noticeUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredNotices.length} notices'); // Debug print
        
        // Sort notices by upload date (newest first)
        ...
        setState(() {
          notices = List<Map<String, dynamic>>.from(filteredNotices);
          isLoading = false;
        });
      }
    } catch (e) {
                          ...
                          child: InkWell(
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  if (notice['first_picture'] != null)
                                    ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: CachedNetworkImage(
                                        imageUrl: '${baseUrl}/storage/${notice['first_picture']['picture_path']}',
                                  ...
                                  Text(
                                    'Uploaded on: ${notice['upload_date']}',
                                    style: TextStyle(
