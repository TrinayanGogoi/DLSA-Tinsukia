  ...
  Future<void> fetchMeetings() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // First, get all tags where meeting is true
        final meetingTags = tags.where((tag) => tag['meeting'] == true).toList();
        
        // Get the uploads_ids from these meeting tags
        final meetingUploadIds = meetingTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${meetingTags.length} meeting tags'); // Debug print
        print('Meeting upload IDs: $meetingUploadIds'); // Debug print

        // Get all uploads that match these ids
        final filteredMeetings = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return meetingUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredMeetings.length} meetings'); // Debug print
        
        // Sort meetings by upload date (newest first)
        ...
        setState(() {
          meetings = List<Map<String, dynamic>>.from(filteredMeetings);
          isLoading = false;
        });
      }
    } catch (e) {
