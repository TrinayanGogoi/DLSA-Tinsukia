...
class LegalLiteracyClassesPage extends StatefulWidget {
  const LegalLiteracyClassesPage({super.key});

  @override
  State<LegalLiteracyClassesPage> createState() => _LegalLiteracyClassesPageState();
}

class _LegalLiteracyClassesPageState extends State<LegalLiteracyClassesPage> {
  List<Map<String, dynamic>> legalLiteracyClasses = [];
  bool isLoading = true;
  ...
  final String baseUrl = Platform.isAndroid 
      ...
      : 'http://localhost:8000'; // iOS Simulator or other platformsther platforms

  @override
  void initState() {
    super.initState();
    fetchLegalLiteracyClasses();
  }

  Future<void> fetchLegalLiteracyClasses() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // Get all tags where legal_literacy_classes is true
        final legalLiteracyClassesTags = tags.where((tag) => tag['legal_literacy_classes'] == true).toList();
        
        // Get the uploads_ids from these legal literacy classes tags
        final legalLiteracyClassesUploadIds = legalLiteracyClassesTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${legalLiteracyClassesTags.length} legal literacy classes tags');
        print('Legal literacy classes upload IDs: $legalLiteracyClassesUploadIds');

        // Get all uploads that match these ids
        final filteredLegalLiteracyClasses = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return legalLiteracyClassesUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredLegalLiteracyClasses.length} legal literacy classes');
        
        // Sort legal literacy classes by upload date (newest first)
        filteredLegalLiteracyClasses.sort((a, b) {
        ...
        setState(() {
          legalLiteracyClasses = List<Map<String, dynamic>>.from(filteredLegalLiteracyClasses);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching legal literacy classes: $e');
      setState(() {
        isLoading = false;
      });
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (legalLiteracyClasses.isEmpty)
                  const Center(child: Text('No legal literacy classes found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    padding: EdgeInsets.zero,
                    itemCount: legalLiteracyClasses.length,
                    itemBuilder: (context, index) {
                      final item = legalLiteracyClasses[index];
                      return Container(
                          ...
                          child: InkWell(
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => DisplaycontentPage(
                                    achievement: item,
                                    categoryName: 'Legal Literacy Classes',
                                    currentIndex: index,
                                    itemsList: legalLiteracyClasses,
                                  ),
                                ),
                              );
                            },
                            child: Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  if (item['first_picture'] != null)
                                    ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: CachedNetworkImage(
                                        imageUrl: '${baseUrl}/storage/${item['first_picture']['picture_path']}',
