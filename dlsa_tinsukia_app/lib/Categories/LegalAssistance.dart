...
class LegalAssistancePage extends StatefulWidget {
  ...
  @override
  State<LegalAssistancePage> createState() => _LegalAssistancePageState();
}

class _LegalAssistancePageState extends State<LegalAssistancePage> {
  List<Map<String, dynamic>> legalAssistanceItems = [];
  bool isLoading = true;
  ...
  void initState() {
    super.initState();
    fetchLegalAssistanceItems();
  }

  Future<void> fetchLegalAssistanceItems() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // Get all tags where legal_assistance is true
        final legalAssistanceTags = tags.where((tag) => tag['legal_assistance'] == true).toList();
        
        // Get the uploads_ids from these legal assistance tags
        final legalAssistanceUploadIds = legalAssistanceTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${legalAssistanceTags.length} legal assistance tags');
        print('Legal assistance upload IDs: $legalAssistanceUploadIds');

        // Get all uploads that match these ids
        final filteredLegalAssistanceItems = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return legalAssistanceUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredLegalAssistanceItems.length} legal assistance items');
        
        // Sort legal assistance items by upload date (newest first)
        filteredLegalAssistanceItems.sort((a, b) {
        ...
        setState(() {
          legalAssistanceItems = List<Map<String, dynamic>>.from(filteredLegalAssistanceItems);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching legal assistance items: $e');
      setState(() {
        isLoading = false;
      });
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (legalAssistanceItems.isEmpty)
                  const Center(child: Text('No legal assistance items found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    padding: EdgeInsets.zero,
                    itemCount: legalAssistanceItems.length,
                    itemBuilder: (context, index) {
                      final item = legalAssistanceItems[index];
                      return Container(
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => DisplaycontentPage(
                                    achievement: item,
                                    categoryName: 'Legal Assistance',
                                    currentIndex: index,
                                    itemsList: legalAssistanceItems,
                                  ),
                                ),
                              );
                            },
