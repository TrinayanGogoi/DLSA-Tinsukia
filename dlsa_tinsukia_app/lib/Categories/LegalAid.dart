...
class _LegalAidPageState extends State<LegalAidPage> {
  List<Map<String, dynamic>> legalAidItems = [];
  bool isLoading = true;
  ...
  Future<void> fetchLegalAidItems() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // Get all tags where legal_aid is true
        final legalAidTags = tags.where((tag) => tag['legal_aid'] == true).toList();
        
        // Get the uploads_ids from these legal aid tags
        final legalAidUploadIds = legalAidTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${legalAidTags.length} legal aid tags');
        print('Legal aid upload IDs: $legalAidUploadIds');

        // Get all uploads that match these ids
        final filteredLegalAidItems = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return legalAidUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredLegalAidItems.length} legal aid items');
        
        // Sort legal aid items by upload date (newest first)
        filteredLegalAidItems.sort((a, b) {
        ...
        setState(() {
          legalAidItems = List<Map<String, dynamic>>.from(filteredLegalAidItems);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching legal aid items: $e');
      setState(() {
        isLoading = false;
      });
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (legalAidItems.isEmpty)
                  const Center(child: Text('No legal aid items found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    padding: EdgeInsets.zero,
                    itemCount: legalAidItems.length,
                    itemBuilder: (context, index) {
                      final item = legalAidItems[index];
                      return Container(
