  ...
  Future<void> fetchRecruitments() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // First, get all tags where recruitment is true
        final recruitmentTags = tags.where((tag) => tag['recruitment'] == true).toList();
        
        // Get the uploads_ids from these recruitment tags
        final recruitmentUploadIds = recruitmentTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${recruitmentTags.length} recruitment tags'); // Debug print
        print('Recruitment upload IDs: $recruitmentUploadIds'); // Debug print

        // Get all uploads that match these ids
        final filteredRecruitments = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return recruitmentUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredRecruitments.length} recruitments'); // Debug print
        
        // Sort recruitments by upload date (newest first)
        ...
        setState(() {
          recruitments = List<Map<String, dynamic>>.from(filteredRecruitments);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching recruitments: $e');
      setState(() {
        isLoading = false;
      });
    }
            ...
            child: Column(
              ...
              children: [
                ...
                if (isLoading)
                  ...
                  const Center(child: Text('No recruitment posts found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                          ...
                          child: InkWell(
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  if (recruitment['first_picture'] != null)
                                    ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: CachedNetworkImage(
                                        imageUrl: '${baseUrl}/storage/${recruitment['first_picture']['picture_path']}',
                                  ...
                                  Text(
                                    'Uploaded on: ${recruitment['upload_date']}',
                                    style: TextStyle(
