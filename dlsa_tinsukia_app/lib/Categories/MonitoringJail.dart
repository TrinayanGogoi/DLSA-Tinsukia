...
class MonitoringJailPage extends StatefulWidget {
  const MonitoringJailPage({super.key});

  @override
  State<MonitoringJailPage> createState() => _MonitoringJailPageState();
}

class _MonitoringJailPageState extends State<MonitoringJailPage> {
  List<Map<String, dynamic>> monitoringJails = [];
  bool isLoading = true;
  ...
  void initState() {
    super.initState();
    fetchMonitoringJails();
  }

  Future<void> fetchMonitoringJails() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // First, get all tags where monitoring_jail is true
        final monitoringJailTags = tags.where((tag) => tag['monitoring_jail'] == true).toList();
        
        // Get the uploads_ids from these monitoring_jail tags
        final monitoringJailUploadIds = monitoringJailTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${monitoringJailTags.length} monitoring jail tags'); // Debug print
        print('Monitoring Jail upload IDs: $monitoringJailUploadIds'); // Debug print

        // Get all uploads that match these ids
        final filteredMonitoringJails = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return monitoringJailUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredMonitoringJails.length} monitoring jails'); // Debug print
        
        // Sort monitoring jails by upload date (newest first)
        filteredMonitoringJails.sort((a, b) {
        ...
        setState(() {
          monitoringJails = List<Map<String, dynamic>>.from(filteredMonitoringJails);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching monitoring jails: $e');
      setState(() {
        isLoading = false;
      });
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (monitoringJails.isEmpty)
                  const Center(child: Text('No monitoring jail records found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    padding: EdgeInsets.zero,
                    itemCount: monitoringJails.length,
                    itemBuilder: (context, index) {
                      final monitoringJail = monitoringJails[index];
                      return Container(
                          ...
                          child: InkWell(
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => DisplaycontentPage(
                                    achievement: monitoringJail,
                                    categoryName: 'Monitoring Jail',
                                    currentIndex: index,
                                    itemsList: monitoringJails,
                                  ),
                                ),
                              );
                            },
                            child: Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  if (monitoringJail['first_picture'] != null)
                                    ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: CachedNetworkImage(
                                        imageUrl: '${baseUrl}/storage/${monitoringJail['first_picture']['picture_path']}',
                                  ...
                                  Text(
                                    'Uploaded on: ${monitoringJail['upload_date']}',
                                    style: TextStyle(
