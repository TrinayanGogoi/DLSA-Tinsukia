...
class MediationPage extends StatefulWidget {
  const MediationPage({super.key});

  @override
  State<MediationPage> createState() => _MediationPageState();
}

class _MediationPageState extends State<MediationPage> {
  List<Map<String, dynamic>> mediations = [];
  bool isLoading = true;
  ...
  Future<void> fetchMediations() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // First, get all tags where mediation is true
        final mediationTags = tags.where((tag) => tag['mediation'] == true).toList();
        
        // Get the uploads_ids from these mediation tags
        final mediationUploadIds = mediationTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${mediationTags.length} mediation tags'); // Debug print
        print('Mediation upload IDs: $mediationUploadIds'); // Debug print

        // Get all uploads that match these ids
        final filteredMediations = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return mediationUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredMediations.length} mediations'); // Debug print
        
        // Sort mediations by upload date (newest first)
        filteredMediations.sort((a, b) {
        ...
        setState(() {
          mediations = List<Map<String, dynamic>>.from(filteredMediations);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching mediations: $e');
      setState(() {
        isLoading = false;
      });
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (mediations.isEmpty)
                  const Center(child: Text('No mediation items found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
