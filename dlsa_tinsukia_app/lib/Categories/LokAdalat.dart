...
class LokAdalatPage extends StatefulWidget {
  const LokAdalatPage({super.key});

  @override
  State<LokAdalatPage> createState() => _LokAdalatPageState();
}

class _LokAdalatPageState extends State<LokAdalatPage> {
  List<Map<String, dynamic>> lokAdalatItems = [];
  bool isLoading = true;
  ...
  Future<void> fetchLokAdalatItems() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // Get all tags where lok_adalat is true
        final lokAdalatTags = tags.where((tag) => tag['lok_adalat'] == true).toList();
        
        // Get the uploads_ids from these lok adalat tags
        final lokAdalatUploadIds = lokAdalatTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${lokAdalatTags.length} lok adalat tags');
        print('Lok adalat upload IDs: $lokAdalatUploadIds');

        // Get all uploads that match these ids
        final filteredLokAdalatItems = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return lokAdalatUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredLokAdalatItems.length} lok adalat items');
        
        // Sort lok adalat items by upload date (newest first)
        filteredLokAdalatItems.sort((a, b) {
        ...
        setState(() {
          lokAdalatItems = List<Map<String, dynamic>>.from(filteredLokAdalatItems);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching lok adalat items: $e');
      setState(() {
        isLoading = false;
      });
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (lokAdalatItems.isEmpty)
                  const Center(child: Text('No Lok Adalat items found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    padding: EdgeInsets.zero,
                    itemCount: lokAdalatItems.length,
                    itemBuilder: (context, index) {
                      final item = lokAdalatItems[index];
                      return Container(
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => DisplaycontentPage(
                                    achievement: item,
                                    categoryName: 'Lok Adalat',
                                    currentIndex: index,
                                    itemsList: lokAdalatItems,
                                  ),
                                ),
                              );
                            },
