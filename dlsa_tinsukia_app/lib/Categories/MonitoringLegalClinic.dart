...
class MonitoringLegalClinicPage extends StatefulWidget {
  const MonitoringLegalClinicPage({super.key});

  @override
  State<MonitoringLegalClinicPage> createState() => _MonitoringLegalClinicPageState();
}

class _MonitoringLegalClinicPageState extends State<MonitoringLegalClinicPage> {
  List<Map<String, dynamic>> monitoringLegalClinics = [];
  bool isLoading = true;
  ...
  void initState() {
    super.initState();
    fetchMonitoringLegalClinics();
  }

  Future<void> fetchMonitoringLegalClinics() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // First, get all tags where monitoring_legal_clinic is true
        final monitoringLegalClinicTags = tags.where((tag) => tag['monitoring_legal_clinic'] == true).toList();
        
        // Get the uploads_ids from these monitoring_legal_clinic tags
        final monitoringLegalClinicUploadIds = monitoringLegalClinicTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${monitoringLegalClinicTags.length} monitoring legal clinic tags'); // Debug print
        print('Monitoring Legal Clinic upload IDs: $monitoringLegalClinicUploadIds'); // Debug print

        // Get all uploads that match these ids
        final filteredMonitoringLegalClinics = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return monitoringLegalClinicUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredMonitoringLegalClinics.length} monitoring legal clinics'); // Debug print
        
        // Sort monitoring legal clinics by upload date (newest first)
        filteredMonitoringLegalClinics.sort((a, b) {
        ...
        setState(() {
          monitoringLegalClinics = List<Map<String, dynamic>>.from(filteredMonitoringLegalClinics);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching monitoring legal clinics: $e');
      setState(() {
        isLoading = false;
      });
    }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (monitoringLegalClinics.isEmpty)
                  const Center(child: Text('No monitoring legal clinic records found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    padding: EdgeInsets.zero,
                    itemCount: monitoringLegalClinics.length,
                    itemBuilder: (context, index) {
                      final monitoringLegalClinic = monitoringLegalClinics[index];
                      return Container(
                          ...
                          child: InkWell(
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => DisplaycontentPage(
                                    achievement: monitoringLegalClinic,
                                    categoryName: 'Monitoring Legal Clinic',
                                    currentIndex: index,
                                    itemsList: monitoringLegalClinics,
                                  ),
                                ),
                              );
                            },
                            child: Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  if (monitoringLegalClinic['first_picture'] != null)
                                    ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: CachedNetworkImage(
                                        imageUrl: '${baseUrl}/storage/${monitoringLegalClinic['first_picture']['picture_path']}',
                                  ...
                                  Text(
                                    'Uploaded on: ${monitoringLegalClinic['upload_date']}',
                                    style: TextStyle(
