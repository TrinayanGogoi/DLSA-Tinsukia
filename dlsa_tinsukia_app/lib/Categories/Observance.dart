...
class ObservancePage extends StatefulWidget {
  const ObservancePage({super.key});

  @override
  State<ObservancePage> createState() => _ObservancePageState();
}

class _ObservancePageState extends State<ObservancePage> {
  List<Map<String, dynamic>> observances = [];
  bool isLoading = true;
  ...
  Future<void> fetchObservances() async {
    try {
      ...
      if (uploadsResponse.statusCode == 200 && 
        ...
        // First, get all tags where observance is true
        final observanceTags = tags.where((tag) => tag['observance'] == true).toList();
        
        // Get the uploads_ids from these observance tags
        final observanceUploadIds = observanceTags.map((tag) => tag['uploads_id'].toString()).toList();

        print('Found ${observanceTags.length} observance tags'); // Debug print
        print('Observance upload IDs: $observanceUploadIds'); // Debug print

        // Get all uploads that match these ids
        final filteredObservances = uploads.where((upload) {
          final uploadId = upload['id'].toString();
          return observanceUploadIds.contains(uploadId);
        }).map((upload) {
        ...
        print('Found ${filteredObservances.length} observances'); // Debug print
        
        // Sort observances by upload date (newest first)
        filteredObservances.sort((a, b) {
        ...
        setState(() {
          observances = List<Map<String, dynamic>>.from(filteredObservances);
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error fetching observances: $e');
      setState(() {
        isLoading = false;
      });
    }
            ...
            child: Column(
              ...
              children: [
                ...
                if (isLoading)
                  ...
                  const Center(child: Text('No observances found'))
                else
                  ListView.builder(
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    padding: EdgeInsets.zero,
                    itemCount: observances.length,
                    itemBuilder: (context, index) {
                      final observance = observances[index];
                      return Container(
                          ...
                          child: InkWell(
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => DisplaycontentPage(
                                    achievement: observance,
                                    categoryName: 'Observance',
                                    currentIndex: index,
                                    itemsList: observances,
                                  ),
                                ),
                              );
                            },
                            child: Padding(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  if (observance['first_picture'] != null)
                                    ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: CachedNetworkImage(
                                        imageUrl: '${baseUrl}/storage/${observance['first_picture']['picture_path']}',
                                  ...
                                  Text(
                                    'Uploaded on: ${observance['upload_date']}',
                                    style: TextStyle(
